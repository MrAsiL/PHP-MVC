		...
		function __construct(){
            try {
                ...
                exit();
		            ...
		            /*SELECT * FROM ayarlar WHERE ID=1 ORDER BY ASC LİMİT 1*/
		public function VeriGetir($tablo,$wherealanlar="",$wherearraydeger="",$orderby="ORDER BY ID ASC",$limit="")
        {
            ...
            else
            {
                ...
                if ($veri!=false && !empty($veri))
                {
                    ...
                    {
                        $datalar[]=$bilgiler;/*sonuc[0] ["baslık"]*/
        ...
        public function ModulEkle()
        {
            if (!empty($_POST["baslik"]))
            {
                ...
                else
                {
                    ...
                    $kategorilekle=$this->SorguCalistir("INSERT INTO kategoriler","SET baslik=?,seflink=?, tablo=?, durum=?, tarih=?",array($baslik,$tablo,"modul",1,date("Y-m-d")));
                    if ($modulekle!=false)
                    {
                        return true;
                    }
            ...
            public function  kategoriGetir($tablo,$secID="",$uz=-1)
            {
                ...
                if ($kategori!=false)
                {
                    for ($q=0;$q<count($kategori);$q++)
                    {
                        ...
                        if ($secID==$kategoriID)
                        {
                            echo '<option value="'.$kategoriID.'" selected="selected">'.str_repeat("&nbsp;&nbsp;&nbsp;",$uz).stripslashes($kategori[$q]
                                ["baslik"]).'</option>';
                        }
                        else
                        {
                            echo '<option value="'.$kategoriID.'">'.str_repeat("&nbsp;&nbsp;&nbsp;",$uz).stripslashes($kategori[$q]
                                ["baslik"]).'</option>';
                        }
